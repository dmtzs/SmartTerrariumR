...
(function ($) {
	...
	function createKeypadContainer() {
		...
		container.setAttribute(
			"name",
			"keyboardContainer" + activeInput.keyboardType
		);
	...
	function createKeypad(set) {
		...
		var layout =
			activeInput.keyboardLayout.layout[activeInput.keyboardType][set];
		...
		for (var i = 0; i < layout.length; i++) {
			...
			button.setAttribute(
				"class",
				"jQKeyboardBtn" + " ui-button-colspan-" + layout[i][3]
			);
        ...
        */
		...
		switch (keyCode) {
			...
			case 190: // dot key
				...
				break;
			default:
				// alpha or numeric key
				newVal = onAlphaNumericKeyPressed(
					currentValue,
					$(clickedBtn),
					caretPos
				);
	...
	function onDotKeyPressed(currentValue, keyElement, caretPos) {
		return insertValueToString(
			currentValue,
			keyElement.attr("data-value"),
			caretPos
		);
	}
